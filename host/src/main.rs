...
struct Args {
    ...
    /// Image ID/Pre state digest hex string
    ///
    /// Must start with 0x
    ///
    /// This will overwrite the default image id (generated by ZKVM) and the one in env
    #[clap(long)]
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let img_id = match (args.image_id, env::var("IMAGE_ID")) {
        (Some(id), _) | (_, Ok(id)) => {
            let (prefix, _) = id.as_str().char_indices().nth(2).expect("Invalid image id");
            id[prefix..].to_string()
        }
        _ => {
